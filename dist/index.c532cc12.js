...
function checkWin() {
    for(let i = 0; i < board.length; i++)for(let e = 0; e < board[0].length; e++){
...
function loadJson() {
    fetch("../json/cards.json").then((response)=>{
        ...
        else alert("No s'ha pogut completar la c\xe0rrega. Error " + response.status);
    }).then((data)=>{
        cards = data["cards"];
    });
}
...
//# sourceMappingURL=index.c532cc12.js.map
